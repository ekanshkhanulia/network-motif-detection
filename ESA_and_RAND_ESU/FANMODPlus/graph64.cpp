...
static const uint64 THR_E_BIT = 0x18UL;
static const uint64 E_BIT_MSK = 0x18UL;
static const short CODESTAMP_BIT = 5;
...
void readHashCode(graph64 &g, graphcode64 gc) {
     ...
     switch ((gc & V_BIT_MSK).convert_to<uint64>()) {
     ...
     gc >>= CODESTAMP_BIT; // remove codestamp
     
     for (int a = g.size-1; a >= 0; --a) {
         for (int b = g.size-1; b >= 0; --b) {
             if (a==b) {
                set_element(g, a, b, (gc & vmsk).convert_to<short>()); 
                gc >>= vbits;         
             } else {
                set_element(g, a, b, (gc & emsk).convert_to<short>());
                gc >>= ebits;                    
             }
         }
     }     
