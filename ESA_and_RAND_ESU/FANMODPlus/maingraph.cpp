	    ...
	    if (!ss.fail()) {
            ...
            if (maxuv > maing.n) {
                ...
                if (gen_dumpfile && (maxuv > 65535)){
                    std::cout << "In Line " << linenumber << ", vertex number " << maxuv << " was specified.\nLargest vertex number allowed with "
                        << " an activated subgraph dump is 65535." << "Exceeded vertex limit \n";
                    return false;
                }
			...
			if (u != v) {
                ...
                if (has_edge_colors && edge_color == 0){
                    std::cout << "In Line " << linenumber << ", edge-color 0"
                        << " was given.\nThis color is not allowed - edges must always " << "have a color >=1." << "Exceeded color limits \n";
                    return false;
                }                
                if (has_edge_colors && maing.num_edge_colors > edge_color_limit){
                    std::cout << "In Line " << linenumber << ", an edge-color >"
                        << " was given.\nLargest edge-color allowed with " << "your settings is " << edge_color_limit << "." << "Exceeded color limits \n";
                    return false;
                }
                if (has_vertex_colors && maing.num_vertex_colors > vertex_color_limit){                    
                    std::cout << "In Line " << linenumber << ", a vertex-color >"
            ...
            std::cout << "Wrong format for input file \'" << filename.c_str() << "\' in line " << linenumber << "\nExpected line format is <int> <int> [<int>] [<int>] [<int>]." << "Error reading file\n";
            return false;
		}
		++linenumber;
	}
    } else { // if !in.good
        std::cout << "Unable to open file \'" << filename.c_str() << "\' for input." << "Error reading file\n";
		return false;
    }
    ++maing.n;
	in.close();
...
void build_graph(maingraph & maing)
{
    ...
    for (std::unordered_map < edge, edgetype >::const_iterator iter = maing.edges.begin();
...
{
	vertex* v_subgraph = new vertex[G_N+1];
	long counter;
	vertex SOURCE_VERTEX;
	vertex FILL_VERTEX;
	long* min_scope = new long[G_N+1];
	long* max_scope = new long[G_N+1];
	long* scope_place = new long[G_N+1];
	short depth;
                ...
                randlib::rand &rand, bool gen_dumpfile, vector<subgraph>& subgraphdump)
{
    ...
    // Init for main loop
	count_subgr = 0;
	vertex v_subgraph[G_N_MAX+1];
	long counter;
	vertex SOURCE_VERTEX; 
	vertex FILL_VERTEX;
	long min_scope[G_N_MAX+1];
	long max_scope[G_N_MAX+1];
	long scope_place[G_N_MAX+1];
	short depth;
    const bool vcolors = (maing.num_vertex_colors > 1);
    const bool ecolors = (maing.num_edge_colors > 1);
    
  	vertex* start_vertices = new vertex[maing.n+2];//##%!
	short scope_place_point[G_N_MAX+1];//##%!
	unsigned long* scope_place_loc[G_N_MAX+1];
